...
public class ContractRenewWorkflow:CamisService
{
    public enum States
    {
        Filing = 1,
    ...
    public ContractRenewWorkflow(IFarmsService service, IWorkflowService workflowService,
     ...
     public void ConfigureMachine()
    {
        Workflow = _workflowService.CreateWorkflow(new WorkflowRequest
        {
            ...
            Description = "Farm Contract Renewal.",
            TypeId = (int)WorkflowTypes.ContractRenewal
        });
    ...
    public void ConfigureMachine(Guid workflowId)
    {
        Workflow = Context.Workflow.First(wf =>
            wf.Id == workflowId && wf.TypeId == (int)WorkflowTypes.ContractRenewal);
    ...
    public void Fire(Guid workflowId,
        StateMachine<States, Triggers>.TriggerWithParameters<ContractRenewalRequest, string, long?> trigger,
        ContractRenewalRequest data,
        string description, long? assignedUser)
    {
    ...
    private void OnRequest(ContractRenewalRequest data, string description, long? assignedUser,
    ...
    private ContractRenewalRequest GetData()
    {
        var workItem = Context.WorkItem.Where(wi => wi.WorkflowId == Workflow.Id).OrderBy(wi => wi.SeqNo)
            .LastOrDefault();

        return workItem != null ? JsonConvert.DeserializeObject<ContractRenewalRequest>(workItem.Data) : null;
    }
    ...
    private void ConfigureAndAddWorkItem(long? role, ContractRenewalRequest data, string description,
        long? assignedUser,
        StateMachine<States, Triggers>.Transition transition)
    {
        ...
        if (data?.SupportiveDocument != null)
        {
            ...
            foreach (var doc in data.SupportiveDocument)
            {
                ...
                if (doc.Document.OverrideFilePath != null &&
                    doc.Document.OverrideFilePath.Contains(pathPrefix))
                {
                    ...
                    if (previousWorkItem != null)
                    {
                        ...
                        var sourceFilePath = Path.Combine(previousPath, $"{doc.Document.Id}");
                        var destFilePath = Path.Combine(fileSavePath, $"{doc.Document.Id}");
                ...
                else if (doc.Document.File != null)
                {
                    doc.Document.Id = doc.Document.Id ?? Guid.NewGuid();
                    var filePath = Path.Combine(fileSavePath, $"{doc.Document.Id}");

                    var fileBytes = Convert.FromBase64String(doc.Document.File);
                    File.WriteAllBytes(filePath, fileBytes);
                    doc.Document.File = null;
                }
                else
                {
                    if (doc.Document.Id != null)
                    {
                        var filePath2 = $"{doc.Document.Id.ToString()}";
                        ...
                        if (File.Exists(filePath2))
                        {
                            ...
                            doc.Document.File = Convert.ToBase64String(fileBytes2);
                        }
                    }
                ...
                doc.Document.OverrideFilePath = $"{pathPrefix}{workItemId}?documentId={doc.Document.Id}";
                doc.DocumentId = doc.Document.Id.ToString();
            }
        }
        ...
        _workflowService.CreateWorkItem(new WorkItemRequest
        {
            ...
            DataType = typeof(ContractRenewalRequest).ToString(),
    ...
    {
        public static StateMachine<States, Triggers>.TriggerWithParameters<ContractRenewalRequest, string, long?> Request;
        ...
        {
            Request = machine.SetTriggerParameters<ContractRenewalRequest, string, long?>(
                Triggers.Request);
