...
export class FarmApiService {
  ...
  searchFarmOperators(term: string, skip: number, take: number): Observable<any> {
    return this.api.get(`Farms/SearchFarmOperators`, {
      params: {
        term: term,
        skip: skip.toString(),
        take: take.toString()
  ...
  searchFarms(term: string, ownerType: number, farmType: number, farmStatus: number, skip: number, take: number): Observable<any> {
    return this.api.get(`Farms/SearchFarms`, {
      params: {
        term: term,
        ownerType: ownerType.toString(),
        farmType: farmType.toString(),
        status: farmStatus.toString(),
        skip: skip.toString(),
        take: take.toString()
      }
    });
  ...
  GetLandRightsByLandIdandFarmId(land_id: string, farm_id: string, part_id: number): Observable<any> {
    return this.api.get(`Farms/GetLandRightsByLandIdandFarmId`, {
      params: {
        landId: land_id,
        farmId: farm_id,
        partId: part_id.toString()
      }
    });
  }
  ...
  GetRightWarning(farm_id: string, land_id: string, part_id: number): Observable<any> {
    return this.api.get(`Farms/GetRightWarning`, {
      params: {
        farmId: farm_id,
        landId: land_id,
        splitIndex: part_id.toString()
      }
    });
  }

  requestContractRenewal(wfid: string, body: any, message: string | null): Observable<any> {
    let params = new HttpParams();
    if (message) {
      params = params.set('description', message);
    }
    return this.api.post(`Farms/RequestContractRenewal/${wfid}`, body, {params});
  }

  approveContractRenewal(workflowId: string, message: string | null): Observable<any> {
    let params = new HttpParams();
    if (message) {
      params = params.set('description', message);
    }
    return this.api.put(`Farms/ApproveContractRenewal/${workflowId}`, null, {params});
  }

  rejectContractRenewal(workflowId: string, message: string | null): Observable<any> {
    let params = new HttpParams();
    if (message) {
      params = params.set('description', message);
    }
    return this.api.put(`Farms/RejectContractRenewal/${workflowId}`, null, {params});
  }

  cancelContractRenewal(workflowId: string, message: string | null): Observable<any> {
    let params = new HttpParams();
    if (message) {
      params = params.set('description', message);
    }
    return this.api.put(`Farms/CancelContractRenewal/${workflowId}`, null, {params});
  }
}
